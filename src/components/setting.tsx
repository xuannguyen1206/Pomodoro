...
function Setting({selected,toggle,updateTimeSettings}:{selected:boolean,toggle:()=>void,updateTimeSettings:(data:timeSetting)=>void}) {
  ...
  return ( 
    <section className={`bg-white w-11/12 h-[90%] absolute rounded-2xl md:h-[70%] md:w-[60%] xl:w-[50%] xl:h-[65%] xl:max-w-[700px] ${selected ? 'block': 'hidden'}`} >
      <div className=" h-[12%] p-4 py-3 border-b-2 border-solid border-black, border-opacity-50 flex items-center justify-between md:p-10">
      ...
      <div>
        <form className=' flex flex-col items-center' onSubmit={handleSubmit(collectInput,onError)}>
          <h1 className=' text-center my-4 text-sm tracking-[0.25rem]'>TIME (MINUTES)</h1>
