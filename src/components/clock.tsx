...
function Clock({keepTab,setKeepTab,taskTab,clockTime,changeClockDoneStatus,isLoadingColor,colors,shuffleColor,changeColor,clockdone}:clockProp) {
  ...
  return (
    <section style={{backgroundColor:`${outsideColor}`}}  className="w-[21.5rem] h-[21.5rem] bg-test-bar rounded-full flex items-center justify-center 
     md:w-[26rem] md:h-[26rem] short:h-[18.5rem] short:w-[18.5rem]">
      <div className="w-[19.8rem] h-[19.8rem] rounded-full relative  flex items-center justify-center md:w-[23rem] md:h-[23rem] short:w-[16.8rem] short:h-[16.8rem]">
        <figure className=" absolute w-full h-full">{/* progress circle */}
          <div className=' relative w-full h-full flex items-center justify-center'> 
            ...
            <div className=" w-[17rem] h-[17rem] bg-white rounded-full relative flex flex-col items-center justify-center text-black md:w-[20rem] md:h-[20rem] short:w-[14.5rem] short:h-[14.5rem]">
                <div className="absolute w-9/12 top-12 flex justify-between md:justify-around short:top-8 short:justify-around short:text-xs">
                  <button onClick={changeColor} className=" p-2 px-6 rounded-full text-white bg-gray-600 text-center short:px-4">
                    {isLoadingColor ? <ReactLoading type={"spin"} color="white" height={'20px'} width={'20px'} /> : 'Color' }
                  </button>
                  <button onClick={shuffleColor} className=" p-2 px-4 rounded-full text-white bg-gray-600 top-12  short:px-4">Shuffle</button>
                </div>
                ...
                <div className="absolute bottom-16 w-1/2 flex justify-around short:bottom-10">
